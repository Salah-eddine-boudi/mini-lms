...
@section('title', 'Modifier : ' . $chapitre->titre)

@section('content')
    <div class="max-w-2xl">
        <a href="{{ route('admin.chapitres.index', ['formation_id' => $chapitre->formation_id]) }}"
           class="text-gray-500 hover:text-gray-700 mb-4 inline-block">
            ← Retour aux chapitres
        </a>

        <div class="bg-white rounded-xl shadow-sm p-6">
            <form method="POST" action="{{ route('admin.chapitres.update', $chapitre) }}">
                @csrf
                @method('PUT')
                    ...
                    <select name="formation_id"
                        ...
                        @foreach($formations as $formation)
                            <option value="{{ $formation->id }}"
                                    {{ old('formation_id', $chapitre->formation_id) == $formation->id ? 'selected' : '' }}>
                                {{ $formation->nom }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="mb-4">
                    ...
                    <textarea name="description"
                              ...
                              rows="3"
                              class="w-full border border-gray-300 rounded-lg px-4 py-2 focus:ring-2 focus:ring-blue-500 focus:border-transparent
                                     @error('description') border-red-500 @enderror">{{ old('description', $chapitre->description) }}</textarea>
                    @error('description')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
