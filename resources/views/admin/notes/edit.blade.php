        ...
        <div class="bg-white rounded-xl shadow-sm p-6">
            <form method="POST" action="{{ route('admin.notes.update', $note) }}">
                ...
                <div class="mb-4">
                    ...
                    <select name="user_id" id="user_id" class="w-full border border-gray-300 rounded-lg px-4 py-2">
                        @foreach($apprenants as $a)
                            <option value="{{ $a->id }}" {{ old('user_id', $note->user_id) == $a->id ? 'selected' : '' }}>{{ $a->prenom }} {{ $a->nom }}</option>
                        @endforeach
                    </select>
                ...
                <div class="mb-4">
                    ...
                    <select name="formation_id" id="formation_id" class="w-full border border-gray-300 rounded-lg px-4 py-2">
                ...
                <div class="mb-4">
                    ...
                    <input type="number" name="note" id="note" value="{{ old('note', $note->note) }}" min="0" max="20" step="0.25"
                ...
                <div class="mb-6">
                    ...
                    <textarea name="commentaire" id="commentaire" rows="3"
                              class="w-full border border-gray-300 rounded-lg px-4 py-2">{{ old('commentaire', $note->commentaire) }}</textarea>
                </div>

                <div class="flex gap-3">
                    <button type="submit" class="bg-blue-600 hover:bg-blue-700 text-white px-6 py-2 rounded-lg transition">Enregistrer</button>
