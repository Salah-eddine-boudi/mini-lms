...
@section('content')
    <div class="max-w-3xl">
        <a href="{{ route('admin.sous-chapitres.index') }}"
           class="text-gray-500 hover:text-gray-700 mb-4 inline-block">
            ← Retour aux sous-chapitres
        </a>

        <div class="bg-white rounded-xl shadow-sm p-6">
            <form method="POST" action="{{ route('admin.sous-chapitres.store') }}">
                @csrf
                    ...
                    <select name="chapitre_id"
                        ...
                        @foreach($chapitres as $chapitre)
                            <option value="{{ $chapitre->id }}"
                                    {{ old('chapitre_id', $selectedChapitre) == $chapitre->id ? 'selected' : '' }}>
                ...
                <div class="mb-4">
                    <label for="contenu" class="block text-sm font-medium text-gray-700 mb-1">
                        Contenu pédagogique <span class="text-gray-400">(HTML ou texte — vous pouvez coller du contenu généré par IA)</span>
                    </label>
